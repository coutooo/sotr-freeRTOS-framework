...
int mainTaskManager( void )
{
    ...
    TMAN_TaskAdd("A");
    TMAN_TaskRegisterAttributes("A", 20,0,5,"N");        // nome,periodo,phase, deadline, precedence constraints   N = NONE  
    
    xTaskCreate(taskBody, ( const signed char * const ) "B", configMINIMAL_STACK_SIZE, (void*) "B", 4, NULL);
    TMAN_TaskAdd("B");
    TMAN_TaskRegisterAttributes("B", 5,0,2,"N");
